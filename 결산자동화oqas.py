...
# 디렉토리 경로 설정
base_directory_path = r"\\192.168.0.228\oqass"  # 네트워크 경로
output_file = "C:\\oqass_output_file.txt"  # 결과 저장 경로
...
def filter_files(file_list):
    ...
    """
    filtered = []
    pattern = re.compile(r".* \d{1,6} (os|od)-\d{2}")  # 'lee, jong min 200603 os-22' 형식의 파일명만 허용
...
def remove_duplicates(file_list):
    """파일명에서 숫자 부분과 구분자까지 고려하여 중복 제거"""
    unique_files = []
    unique_identifiers = set()

    for file_name in file_list:
        # 파일명에서 '이름 숫자' 형식으로 고유 식별자를 생성
        match = re.search(r"(.+ \d{1,6}) (os|od)-\d{2}", file_name)
        if match:
...
def get_today_folder_path(base_directory_path):
    ...
    today_folder = today.strftime("%Y\\%m\\%m.%d")  # '2025\\01\\01.22' 형식으로 수정
    today_folder_path = os.path.join(base_directory_path, today_folder)
