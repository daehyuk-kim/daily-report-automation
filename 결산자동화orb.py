...
# 디렉토리 경로 설정
base_directory_path = r"\\192.168.0.209\orb"  # 네트워크 경로
output_file = "C:\\orb_output_file.txt"  # 결과 저장 경로
...
def filter_files(file_list):
    ...
    """
    filtered = []
    pattern = re.compile(r".* \d{1,6} .+")  # 숫자 1~6자리가 포함된 파일명만 허용
...
def remove_duplicates(file_list):
    ...
    for file_name in file_list:
        # 파일명에서 1~6자리 숫자와 구분자(`od-xx`, `os-xx`)까지 포함된 부분을 기준으로 고유 식별자 생성
        match = re.search(r"(.+ \d{1,6}) (od|os)-\d{2}", file_name)
        if match:
...
def get_today_folder_path(base_directory_path):
    ...
    today_folder = today.strftime("%Y.%m\\ORB %m.%d")  # '2025.01' 형식으로 수정
