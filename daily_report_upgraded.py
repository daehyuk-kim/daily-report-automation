...
try:
    ...
    HAS_WATCHDOG = False
    print("경고: watchdog가 설치되지 않았습니다. 실시간 모니터링을 사용할 수 없습니다.")
...
class DailyReportSystem:
    ...
    def get_hospital_schedule_staff(self, target_date: date, department: str = "검사실") -> List[str]:
        """Hospital Schedule API에서 근무 인원 가져오기"""
        try:
            ...
            print(f"Hospital Schedule API 오류: {e}")
            return []
...
class CleanWhiteGUI:
    ...
    def setup_gui(self):
        """GUI 구성"""
        self.root.title("일일결산 자동화 시스템 (Upgraded)")
    ...
    def load_staff_from_schedule(self):
        """Hospital Schedule API에서 직원 로드"""
        def load_thread():
            try:
                target_date = datetime.strptime(self.date_entry.get(), '%Y-%m-%d').date()
                staff_list = self.system.get_hospital_schedule_staff(target_date, "검사실")

                if staff_list:
                    staff_text = ", ".join(staff_list) + f" ({len(staff_list)}명)"
                else:
                    staff_text = "API 연결 실패 또는 근무자 없음"

                self.root.after(0, lambda: self.staff_label.config(text=staff_text, fg="#11998e"))
            except Exception as e:
                self.root.after(0, lambda: self.staff_label.config(text=f"오류: {e}", fg="#e94560"))
    ...
    def toggle_monitoring(self):
        """실시간 모니터링 토글"""
        if not HAS_WATCHDOG:
            messagebox.showwarning("경고", "watchdog가 설치되지 않았습니다.")
    ...
    def start_monitoring(self):
        ...
        self.observer.start()
        messagebox.showinfo("안내", "실시간 모니터링이 시작되었습니다.")
    ...
    def on_file_created(self, file_path):
        """파일 생성 감지 시 자동 스캔"""
        # 자동 스캔 트리거
        self.root.after(2000, self.run_scan)  # 2초 후 스캔
